...
def search_google(query):
    ...
    try:
        ...
        return f"❌ 크롤링 구동 중 치명적 오류 발생: {str(e)}"

    if not crawled_text.strip():
        return "❌ 커뮤니티 페이지에서 유효한 텍스트를 추출하지 못했습니다. (접속 차단 혹은 빈 페이지)"

    print("  -> [Gemini] 수집된 데이터 맞춤형 구조 분석 중...")
    ...
    2. 만약 향수, 정보성 질문 등 다른 성격의 질문이라면 그에 가장 알맞은 최적의 구조로 유연하게 답변해주세요.
